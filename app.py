...
def show_image(image, grid):
    ...
    for i in range(16):
        if(grid[i]):
            ...
            rect = patches.Rectangle((col * 30, row * 30), 30, 30, linewidth=1, edgecolor='#1a73e8', facecolor='#1a73e8', alpha=0.3)
            ax.add_patch(rect)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image_name': sample['image_name'],
...
class GridPointsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_name = os.path.normpath(os.path.join(self.image_dir, self.category, f"{self.category} ({self.grids[index][0]}).png"))
...
def show_grid_batch(sample_batched):
    images_batch, grid_batch = \
            sample_batched['image'], sample_batched['grid']
    ...
    for i in range(batch_size):
        for j in range(16):
            ...
            if(grid_batch[i][j].item()):
                ...
                rect = patches.Rectangle((col * 30 + 123 * i, row * 30), 30, 30, linewidth=1, edgecolor='#1a73e8', facecolor='#1a73e8', alpha=0.3)
                ax.add_patch(rect)
