...
def grid(category):
    label_dir = f"data/labels/{category}"
    ...
    # check if _ndarrays directory exists in data/labels
    if not os.path.exists("recaptcha-data/labels"):
        os.makedirs("recaptcha-data/labels")
    np.save(f"recaptcha-data/labels/{category}.npy", np_grids)
